    ...
    function SignalObject:Fire(...)
        ...
        while ConnectionIndex <= #SignalObject.ActiveConnections do
            ...
            if ConnectionObject.Connected then
                local ExecutionSuccess, _ = pcall(ConnectionObject.Function, ...)
                if not ExecutionSuccess then
                    Error_Tracking_Current_Count += 1
...
function RunService:BindToRenderStep(BindName, BindPriority, BindFunction)
    if type(BindName) ~= "string" or type(BindFunction) ~= "function" then return end
...
task.spawn(function()
    while Thread_Execution_Active_State do
        local ok = pcall(function()
            local now   = os.clock()
            local delta = math.min(now - Performance_Last_Tick_Timestamp, 1)
            Performance_Last_Tick_Timestamp = now
            Metrics_Accumulated_Frame_Counter += 1
            if Thread_Execution_Active_State then RunService.Stepped:Fire(now, delta) end
            if Thread_Execution_Active_State then
                local count = 0
                for _ in pairs(Render_Step_Priority_Bindings) do count += 1 end
                if count ~= Cache_Validated_Bind_Count then
                    ...
                    for _, data in pairs(Render_Step_Priority_Bindings) do
                        if data and type(data.Function) == "function" then
                            table.insert(Cache_Sorted_Binding_Registry, data)
                        end
                    end
                    table.sort(Cache_Sorted_Binding_Registry, function(a,b) return a.Priority < b.Priority end)
                    Cache_Validated_Bind_Count = count
                end
                for i = 1, #Cache_Sorted_Binding_Registry do
                    if not Thread_Execution_Active_State then break end
                    local b = Cache_Sorted_Binding_Registry[i]
                    if b and b.Function then pcall(b.Function, delta) end
                end
            end
            if Thread_Execution_Active_State then RunService.RenderStepped:Fire(delta) end
            if Thread_Execution_Active_State then RunService.Heartbeat:Fire(delta) end
        end)
        if not ok then
            Error_Tracking_Current_Count += 1
            if Error_Tracking_Current_Count >= Error_Handling_Max_Threshold_Limit then
        ...
        if Thread_Execution_Active_State then task.wait() end
    end
end)
                                        ...
                                        do
                                            ...
                                            function ui:step()
                                                ...
                                                if self.watermark_enabled ~= false then
                                                    ...
                                                    if not self._wm_x then self._wm_x = workspace.CurrentCamera.ViewportSize.X - box_size.X - 20 end
                                                    if not self._wm_y then self._wm_y = 20 end
                                                    local pos = v2(self._wm_x, self._wm_y)
...
local lib = ui:create("mm2 autofarm", {
    theme = "matcha",
    size  = Vector2.new(500, 300)
})

local tabFarm     = lib:tab("autofarm")
local tabSettings = lib:tab("settings")

local secFarm = tabFarm:section("coin farm", false)
...
local secUI = tabSettings:section("ui settings", false)
...
timerBG.Filled       = true
timerBG.Color        = Color3.fromRGB(24, 24, 24)
timerBG.Size         = Vector2.new(TW, TH)
timerBG.ZIndex       = 2000
timerBG.Transparency = 1
timerBG.Visible      = false

local timerOutline = Drawing.new("Square")
timerOutline.Filled       = false
timerOutline.Color        = Color3.fromRGB(12, 12, 12)
timerOutline.Size         = Vector2.new(TW + 2, TH + 2)
timerOutline.ZIndex       = 1999
timerOutline.Thickness    = 1
timerOutline.Transparency = 1
timerOutline.Visible      = false

local timerAccent = Drawing.new("Square")
timerAccent.Filled       = true
timerAccent.Color        = Color3.fromRGB(64, 160, 110)
timerAccent.Size         = Vector2.new(TW, 3)
timerAccent.ZIndex       = 2001
timerAccent.Transparency = 1
timerAccent.Visible      = false

local timerTitle = Drawing.new("Text")
timerTitle.Text         = "ROUND TIMER"
timerTitle.Color        = Color3.fromRGB(140, 140, 140)
timerTitle.Size         = 13
timerTitle.Font         = Drawing.Fonts.UI
timerTitle.Outline      = false
timerTitle.ZIndex       = 2002
timerTitle.Transparency = 1
timerTitle.Visible      = false

local timerLabel = Drawing.new("Text")
timerLabel.Text         = "--:--"
timerLabel.Color        = Color3.fromRGB(230, 230, 230)
timerLabel.Size         = 20
timerLabel.Font         = Drawing.Fonts.UI
timerLabel.Outline      = false
timerLabel.ZIndex       = 2002
...
local function applyTimerPos()
    ...
    timerBG.Position      = Vector2.new(px, py)
    timerOutline.Position = Vector2.new(px - 1, py - 1)
    timerAccent.Position  = Vector2.new(px, py)
    timerTitle.Position   = Vector2.new(px + 10, py + 7)
    timerLabel.Position   = Vector2.new(px + 10, py + 22)
end
...
local function mouseInBox(bx, by, bw, bh)
...
while lib.running do
    ...
    if pressed and not wasPressed then
        ...
        if timerBG.Visible and mouseInBox(timerX, timerY, TW, TH) then
            ...
            dragOffX = mx - timerX
            dragOffY = my - timerY
        elseif lib._wm_x and lib._wm_y then
            local wmW = lib.watermark and lib.watermark.container and lib.watermark.container.Size.X or 160
            if mouseInBox(lib._wm_x, lib._wm_y, wmW, 30) then
                dragging = "watermark"
                dragOffX = mx - lib._wm_x
                dragOffY = my - lib._wm_y
            end
        end
    ...
    if dragging == "timer" then
        timerX = mx - dragOffX
        timerY = my - dragOffY
    elseif dragging == "watermark" then
        lib._wm_x = mx - dragOffX
        lib._wm_y = my - dragOffY
    end
    ...
    if timerPart then
        ...
        if lib.colors then timerAccent.Color = lib.colors.accent end
        timerBG.Visible      = true
        timerOutline.Visible = true
        timerAccent.Visible  = true
